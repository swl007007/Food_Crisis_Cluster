...
echo ===== Starting XGBoost Monthly-by-Month Batch Processing (2021-2024, Visual Debug Mode) =====
echo This script will run XGBoost processing ONE MONTH AT A TIME
...
echo   2. app/main_model_XGB.py: track_partition_metrics = True
echo   3. app/main_model_XGB.py: enable_metrics_maps = True
echo.
...
echo.
echo Visual outputs archived in folders: result_GeoXGB_YEAR_fsN_YYYY-MM_visual
echo Combined results: results_df_xgb_gp_fsN_YEAR_YEAR.csv (12 months combined)
echo.
echo All temporary result_GeoXGB_* directories have been cleaned up.
...
echo Running: python app/main_model_XGB.py --start_year %proc_year% --end_year %proc_year% --forecasting_scope %proc_scope%
echo Environment: DESIRED_TERMS=%DESIRED_TERMS%
echo Visual outputs will be saved to: result_GeoXGB_*/vis/

REM Run the Python script for this single month
python app/main_model_XGB.py --start_year %proc_year% --end_year %proc_year% --forecasting_scope %proc_scope%
...
set "monthly_result_file=monthly_results\results_df_xgb_gp_fs%ext_scope%_%ext_year%_%ext_month%.csv"
set "monthly_pred_file=monthly_results\y_pred_test_xgb_gp_fs%ext_scope%_%ext_year%_%ext_month%.csv"

REM Find the most recent results file (should be from current run)
set "source_result_file="
for %%F in (results_df_xgb_gp_fs%ext_scope%_*.csv) do (
    set "source_result_file=%%F"
)

REM Find the most recent prediction file
set "source_pred_file="
for %%F in (y_pred_test_xgb_gp_fs%ext_scope%_*.csv) do (
...
REM Output files
set "combined_result_file=results_df_xgb_gp_fs%comb_scope%_%comb_year%_%comb_year%.csv"
set "combined_pred_file=y_pred_test_xgb_gp_fs%comb_scope%_%comb_year%_%comb_year%.csv"

REM Use Python to combine the monthly CSV files
python -c "import pandas as pd; import glob; pattern='monthly_results/results_df_xgb_gp_fs%comb_scope%_%comb_year%_*.csv'; files=sorted(glob.glob(pattern)); df=pd.concat([pd.read_csv(f) for f in files], ignore_index=True); df.to_csv('%combined_result_file%', index=False); print(f'Combined {len(files)} monthly result files')"

python -c "import pandas as pd; import glob; pattern='monthly_results/y_pred_test_xgb_gp_fs%comb_scope%_%comb_year%_*.csv'; files=sorted(glob.glob(pattern)); df=pd.concat([pd.read_csv(f) for f in files], ignore_index=True); df.to_csv('%combined_pred_file%', index=False); print(f'Combined {len(files)} monthly prediction files')"
...
del /q "monthly_results\results_df_xgb_gp_fs%comb_scope%_%comb_year%_*.csv" 2>nul
del /q "monthly_results\y_pred_test_xgb_gp_fs%comb_scope%_%comb_year%_*.csv" 2>nul
...
REM Find the result_GeoXGB directory - check common patterns in priority order
REM Priority: result_GeoXGB (no number), result_GeoXGB_0, result_GeoXGB_1, result_GeoXGB_2, etc.
set "source_dir="

REM First check result_GeoXGB (no number suffix)
if exist "result_GeoXGB" (
    if exist "result_GeoXGB\vis" (
        set "source_dir=result_GeoXGB"
        goto :found_source_dir
    )
)

REM Then check numbered versions: result_GeoXGB_0 through result_GeoXGB_20
for /L %%N in (0,1,20) do (
    if exist "result_GeoXGB_%%N" (
        if exist "result_GeoXGB_%%N\vis" (
            set "source_dir=result_GeoXGB_%%N"
            goto :found_source_dir
        )
    )
)

REM If still not found, search for any result_GeoXGB_* that's NOT archived (doesn't end with _visual)
for /d %%D in (result_GeoXGB_*) do (
    set "temp_dir=%%D"
...
echo WARNING: No valid result_GeoXGB* directory with /vis folder found to archive
goto :eof
...
set "archive_folder=result_GeoXGB_%archive_year%_fs%archive_scope%_%archive_year%-%archive_month%_visual"
...
for /d %%D in (result_GeoXGB*) do (
    call :check_and_delete_xgb_folder "%%D"
)
...
if exist "result_GeoXGB" (
    call :check_and_delete_xgb_folder "result_GeoXGB"
)

for /L %%i in (0,1,20) do (
    if exist "result_GeoXGB_%%i" (
        call :check_and_delete_xgb_folder "result_GeoXGB_%%i"
    )
)
