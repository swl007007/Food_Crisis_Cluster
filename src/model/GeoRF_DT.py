...
from src.model.model_DT import DTmodel, save_single, predict_test_group_wise#model is easily customizable
...
class GeoRF_DT():
    ...
    #X_group stores group assignment: customizable. In this example, groups are defined by grid cells in space.
		...
		self.model = DTmodel(self.dir_ckpt, self.n_trees_unit, max_depth=self.max_depth)
		self.model.train(
			X[train_list_init],
			y[train_list_init],
			branch_id='',
	...
	def evaluate_2layer(self, X_L1_test, X_L2_test, y_test, X_group_test, 
		...
		# Train base Layer 2 (global DT for error correction)
		error_targets_base = (y_train != y_pred_base_l1_train).astype(int)
		base_l2 = DecisionTreeClassifier(
			max_depth=self.max_depth,
			random_state=self.random_state
		)
		...
		print('2-Layer Base DT F1:', f1_base)
