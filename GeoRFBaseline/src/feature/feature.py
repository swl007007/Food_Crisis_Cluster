...
def prepare_features(df, X_group, X_loc, forecasting_scope=len(ACTIVE_LAGS)):
    ...
    if not pd.MultiIndex.from_frame(df[["FEWSNET_admin_code", "date"]]).is_monotonic_increasing:
        raise ValueError("Input rows must be sorted by FEWSNET_admin_code and date before creating spatial groups.")
...
def create_correspondence_table(df, years, dates, train_year, month, X_branch_id, result_dir,
    ...
    try:
        ...
        correspondence_table['partition_id'] = correspondence_table['partition_id'].astype(str).replace('', 'root')
