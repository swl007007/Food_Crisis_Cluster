...
def partition(model, X, y,
  ...
  **paras is for model-specific parameters (could be different for deep learning and traditional ML)
  ...
  f1_mode = MODE == 'classification' and CRISIS_FOCUSED_OPTIMIZATION and GOVERNING_METRIC == 'class_1_f1'
  if f1_mode:
    branch_table[0, 0] = 1  # Every descendant needs an accepted parent, even below min_depth.
  else:
    branch_table[:,0:min_depth] = 1
  ...
  for i in range(max_depth-1):
    ...
    for j in range(num_branches):
      ...
      labels = np.argmax(y_val, axis=1) if np.ndim(y_val) > 1 else np.asarray(y_val)
      selected_support = np.sum(labels == 1) if f1_mode else np.sum(y_val_value)
      if selected_support <= MIN_BRANCH_SAMPLE_SIZE:
      ...
      if any(len(rows) == 0 for rows in (X0_train, X1_train, X0_val, X1_val)):
        print("Empty candidate child; keeping parent.")
        continue
      ...
      if f1_mode:
        ...
        accepted, selected, predictions, base_f1, split_f1 = select_f1_children(
            y0_val, y1_val, parent0, parent1, y0_pred, y1_pred,
            min_improvement=MIN_CLASS_1_IMPROVEMENT_THRESHOLD,
        )
        sig = int(accepted)
        if accepted:
        ...
        print(f"F1 performance gate: parent={base_f1:.6f}, candidate={split_f1:.6f}, accepted={bool(sig)}")
      else:
        sig = 1
