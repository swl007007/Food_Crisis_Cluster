...
def get_metric_score_array(y_true, y_pred, metric_name=None):
    ...
    if metric_name == 'class_1_f1':
        raise ValueError("F1 is an aggregate metric; use get_class_1_f1_score.")
...
def get_class_wise_stat(y_true, y_pred, y_group, mode = MODE, onehot = ONEHOT):
  # F1 exposure D and numerator A are additive; their ratio is aggregate F1.
  if mode == 'classification' and CRISIS_FOCUSED_OPTIMIZATION and GOVERNING_METRIC == 'class_1_f1':
    truth = np.argmax(y_true, axis=-1) if np.ndim(y_true) > 1 else np.asarray(y_true)
    pred = np.argmax(y_pred, axis=-1) if np.ndim(y_pred) > 1 else np.asarray(y_pred)
    if truth.shape != pred.shape or truth.ndim != 1:
      raise ValueError("Binary F1 requires aligned one-dimensional labels.")
    if not np.isin(truth, [0, 1]).all() or not np.isin(pred, [0, 1]).all():
      raise ValueError("This baseline supports binary class-1 F1 only.")
    tp = ((truth == 1) & (pred == 1)).astype(int)
    errors = (truth != pred).astype(int)
    groups, counts = groupby_sum(np.column_stack((2 * tp + errors, 2 * tp)), y_group)
    return groups, counts[:, :1], groups, counts[:, 1:]
...
def get_c_b(y_true_value, true_pred_value):
  """Return observed and expected loss; F1 inputs are D and 2TP."""
  c = y_true_value - true_pred_value
  total = np.sum(y_true_value, axis=0, keepdims=True)
  fraction = np.divide(y_true_value, total, out=np.zeros_like(y_true_value, dtype=float), where=total > 0)
  return c, np.sum(c, axis=0, keepdims=True) * fraction
...
def get_top_cells(g, flex = FLEX_OPTION, flex_ratio = FLEX_RATIO, flex_type = FLEX_TYPE, cnt = None):
  ...
  if len(g) < 2:
    return np.arange(len(g)), np.array([], dtype=int)
...
def select_f1_children(y0, y1, parent0, parent1, child0, child1, min_improvement=0.01):
    """Choose existing parent/child checkpoints by pooled validation F1.

    Parent wins ties. The returned decision is a performance gate, not a p-value.
    """
    truth = np.concatenate((y0, y1))
    truth = np.argmax(truth, axis=1) if truth.ndim > 1 else truth

    def exact_f1(pred):
        labels = np.argmax(pred, axis=1) if pred.ndim > 1 else pred
        tp2 = 2 * int(np.sum((truth == 1) & (labels == 1)))
        denominator = tp2 + int(np.sum(truth != labels))
        return Fraction(tp2, denominator) if denominator else Fraction(0)

    base = exact_f1(np.concatenate((parent0, parent1)))
    ...
    for use0, use1 in ((True, False), (False, True), (True, True)):
        ...
        score = exact_f1(np.concatenate((pred0, pred1)))
        if score > best:
            best, choice, predictions = score, (use0, use1), (pred0, pred1)
    # Count-based rational arithmetic keeps the strict boundary exact.
    accepted = best - base > Fraction(str(min_improvement))
    return accepted, choice, predictions, float(base), float(best)


def get_score(y_true, y_pred, mode = MODE):
  if mode == 'classification' and CRISIS_FOCUSED_OPTIMIZATION and GOVERNING_METRIC == 'class_1_f1':
    return get_class_1_f1_score(y_true, y_pred)
...
def scan(y_true_value, true_pred_value, min_sample,
  ...
  q_init = np.divide(c, b, out=np.zeros_like(c, dtype=float), where=b > 0)
  for i in range(q_init.shape[1]):
    ...
    expected = np.sum(b[s_class, i])
    q[i] = np.sum(c[s_class, i]) / expected if expected > 0 else 1.0
  ...
  for i in range(max_iteration):#coordinate descent
    ...
    #update q
    expected = np.sum(b[s0], axis=0)
    q = np.divide(np.sum(c[s0], axis=0), expected, out=np.ones_like(expected, dtype=float), where=expected > 0)
    q[q == 0] = 1
    q[q_filter == 1] = 1
