...
* `manifest.json -> stages.stage1_fit.split_gate` — the effective gate, whether it
  came from the CLI override or the pinned default, and the readback from all three
  consuming namespaces. Recorded for every cell, not only the overridden ones.
...
Regenerate the verification artifacts from a clean checkout:

```
python IPCCHGeoRFExperiment/check_cells.py \
    --out IPCCHGeoRFExperiment/runs/cell_verification.json
python IPCCHGeoRFExperiment/check_same_key_reference.py \
    --out IPCCHGeoRFExperiment/runs/same_key_reference.json
```

Both exit non-zero on any failure and read only committed artifacts, so they run in a
fresh clone. `check_cells.py` covers R2 settings drift, R3 gate readback, R4 cohort
reconciliation and R6 candidate evaluation counts; `check_same_key_reference.py` is the
R5 recomputation and verifies the key sets match before computing anything.
