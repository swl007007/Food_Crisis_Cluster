...
def run_temporal_evaluation(X, y, X_loc, X_group, years, dates, l1_index, l2_index, feature_columns,
    ...
    # Import monthly evaluation configuration
    ...
    if DESIRED_TERMS is not None and len(DESIRED_TERMS) > 0:
        ...
        progress_bar = tqdm(total=len(months_to_evaluate), desc="GeoXGB Monthly Evaluation", unit="month")
        ...
        for i, test_month_period in enumerate(months_to_evaluate):
            ...
            progress_bar.set_description(f"GeoXGB {test_month_period}")
            ...
            try:
                # Train-test split with monthly rolling window
                ...
                if nowcasting:
                    ...
                    print(f"{test_month_period} Test - 2-Layer GeoXGB F1: {f1}, 2-Layer Base XGB F1: {f1_base}")
                ...
                print(f"{test_month_period} Test - GeoXGB F1: {f1}, Base XGB F1: {f1_base}")
    ...
    # === LEGACY QUARTERLY MODE FALLBACK ===
        ...
        print("\n[DEPRECATION WARNING] DESIRED_TERMS not configured.")
        print("Using legacy quarterly mode is deprecated. Configure DESIRED_TERMS in config_visual.py for monthly evaluation.\n")
        raise NotImplementedError("Legacy quarterly mode not implemented. Please configure DESIRED_TERMS in config_visual.py.")
