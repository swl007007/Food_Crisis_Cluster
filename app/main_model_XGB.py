...
if ACTIVE_LAGS != EXPECTED_LAG_SCHEDULE:
    raise ValueError(
        f"GeoXGB requires FEWSNET-aligned lags {EXPECTED_LAG_SCHEDULE}; found {ACTIVE_LAGS}."
    )
...
# Configuration
ARTIFACTS_ROOT = os.path.join('result_GeoXGB')
...
def run_temporal_evaluation(X, y, X_loc, X_group, years, dates, l1_index, l2_index, feature_columns,
    ...
    # === LEGACY QUARTERLY MODE FALLBACK ===
        ...
        print("Using legacy quarterly mode is deprecated. Configure DESIRED_TERMS in config.py for monthly evaluation.\n")
        raise NotImplementedError("Legacy quarterly mode not implemented. Please configure DESIRED_TERMS in config.py.")
...
def main():
    ...
    try:
        ...
        class_1_columns = [col for col in results_df.columns if '(1)' in col or col in ['year', 'month']]
