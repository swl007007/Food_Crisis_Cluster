...
if not IS_DRY_RUN:
    from src.model.adapters import GFAdapter
    ...
    MODEL_ADAPTER = GFAdapter()
else:
    class DryRunAdapter:
        def __init__(self) -> None:
            self.key = 'gf'
            self.display_name = 'GeoRF'
            self.baseline_label = 'RF'
...
def main():
    ...
    scope_help = \
        "Forecasting scope. " \
        f"0=fs0 (lag=1, stand-alone); 1..{len(ACTIVE_LAGS)}=fs1..fs{len(ACTIVE_LAGS)} for lag schedule " \
